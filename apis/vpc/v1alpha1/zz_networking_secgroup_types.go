...
type networking_secgroupInitParameters struct {
...
type networking_secgroupObservation struct {
...
type networking_secgroupParameters struct {
...
// networking_secgroupSpec defines the desired state of networking_secgroup
type networking_secgroupSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     networking_secgroupParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider networking_secgroupInitParameters `json:"initProvider,omitempty"`
}

// networking_secgroupStatus defines the observed state of networking_secgroup.
type networking_secgroupStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        networking_secgroupObservation `json:"atProvider,omitempty"`
}
...
// networking_secgroup is the Schema for the networking_secgroups API. ""
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,huaweicloud}
type networking_secgroup struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.name) || (has(self.initProvider) && has(self.initProvider.name))",message="spec.forProvider.name is a required parameter"
	Spec   networking_secgroupSpec   `json:"spec"`
	Status networking_secgroupStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// networking_secgroupList contains a list of networking_secgroups
type networking_secgroupList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []networking_secgroup `json:"items"`
}

// Repository type metadata.
var (
	networking_secgroup_Kind             = "networking_secgroup"
	networking_secgroup_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: networking_secgroup_Kind}.String()
	networking_secgroup_KindAPIVersion   = networking_secgroup_Kind + "." + CRDGroupVersion.String()
	networking_secgroup_GroupVersionKind = CRDGroupVersion.WithKind(networking_secgroup_Kind)
)

func init() {
	SchemeBuilder.Register(&networking_secgroup{}, &networking_secgroupList{})
}
