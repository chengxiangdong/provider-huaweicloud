...
type obs_bucketInitParameters struct {
...
type obs_bucketObservation struct {
...
type obs_bucketParameters struct {
...
// obs_bucketSpec defines the desired state of obs_bucket
type obs_bucketSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     obs_bucketParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider obs_bucketInitParameters `json:"initProvider,omitempty"`
}

// obs_bucketStatus defines the observed state of obs_bucket.
type obs_bucketStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        obs_bucketObservation `json:"atProvider,omitempty"`
}
...
type obs_bucket struct {
	...
	Spec   obs_bucketSpec   `json:"spec"`
	Status obs_bucketStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// obs_bucketList contains a list of obs_buckets
type obs_bucketList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []obs_bucket `json:"items"`
}

// Repository type metadata.
var (
	obs_bucket_Kind             = "obs_bucket"
	obs_bucket_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: obs_bucket_Kind}.String()
	obs_bucket_KindAPIVersion   = obs_bucket_Kind + "." + CRDGroupVersion.String()
	obs_bucket_GroupVersionKind = CRDGroupVersion.WithKind(obs_bucket_Kind)
)

func init() {
	SchemeBuilder.Register(&obs_bucket{}, &obs_bucketList{})
}
