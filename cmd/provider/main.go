...
import (
	...
	"github.com/chengxiangdong/provider-huaweicloud/internal/features"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "huaweicloud support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-huaweicloud"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-huaweicloud",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add huaweicloud APIs to scheme")
	...
	kingpin.FatalIfError(huaweicloud.Setup(mgr, o), "Cannot setup huaweicloud controllers")
